    ...
    >
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          <div class="mb-3 flex items-center justify-between gap-3">
            ...
            <button
              ...
              type="button"
              @click="importOpenAIModelsForCreate"
              :disabled="openAIModelImporting"
              class="btn btn-secondary btn-sm"
            >
              {{ openAIModelImporting ? t('admin.accounts.importingModels') : t('admin.accounts.importModels') }}
            </button>
          </div>
      ...
      >
        <div class="mb-3 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            @click="importOpenAIModelsForOAuthCreate"
            :disabled="openAIModelImporting"
            class="btn btn-secondary btn-sm"
          >
            {{ openAIModelImporting ? t('admin.accounts.importingModels') : t('admin.accounts.importModels') }}
...
const openAIModelImporting = ref(false)
...
const mergeImportedModels = (models: string[]) => {
  const merged = [...allowedModels.value]
...
const importOpenAIModelsForCreate = async () => {
  if (form.platform !== 'openai') return
  openAIModelImporting.value = true
  try {
    const result = await adminAPI.accounts.importOpenAIModels({
      platform: 'openai',
      type: 'apikey',
    ...
    mergeImportedModels(result.models)
    if (result.fallback) {
      appStore.showInfo(t('admin.accounts.importModelsFallback', { count: result.models.length }))
    } else {
      appStore.showSuccess(t('admin.accounts.importModelsSuccess', { count: result.models.length }))
    }
  } catch (error: any) {
    appStore.showError(error?.message || t('admin.accounts.failedToLoadModels'))
  } finally {
    openAIModelImporting.value = false
  }
}

const importOpenAIModelsForOAuthCreate = async () => {
  if (form.platform !== 'openai' || accountCategory.value !== 'oauth-based') return
  openAIModelImporting.value = true
  try {
    const result = await adminAPI.accounts.importOpenAIModels({
      platform: 'openai',
