      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          <div class="card">
            ...
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t("admin.settings.urlAllowlist.title") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t("admin.settings.urlAllowlist.description") }}
              </p>
            </div>
            <div class="space-y-5 p-6">
              <div class="flex items-center justify-between">
                <div>
                  <label class="font-medium text-gray-900 dark:text-white">{{
                    t("admin.settings.urlAllowlist.enabled")
                  }}</label>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ t("admin.settings.urlAllowlist.enabledHint") }}
                  </p>
                </div>
                <Toggle v-model="form.url_allowlist_enabled" />
              </div>

              <div class="border-t border-gray-100 pt-4 dark:border-dark-700">
                <label class="font-medium text-gray-900 dark:text-white">{{
                  t("admin.settings.urlAllowlist.upstreamHosts")
                }}</label>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  {{ t("admin.settings.urlAllowlist.upstreamHostsHint") }}
                </p>
                <textarea
                  v-model="urlAllowlistUpstreamHostsInput"
                  rows="6"
                  class="input mt-3 font-mono"
                  :placeholder="
                    t('admin.settings.urlAllowlist.upstreamHostsPlaceholder')
                  "
                ></textarea>
              </div>
            </div>
...
const urlAllowlistUpstreamHostsInput = ref("");
...
const form = reactive<SettingsForm>({
  ...
  url_allowlist_enabled: false,
  url_allowlist_upstream_hosts: [],
  default_balance: 0,
...
function parseURLAllowlistHostsInput(input: string): string[] {
  return input
    .split(/[,\n\r，]+/)
    .map((item) => item.trim())
    .filter(Boolean);
...
async function loadSettings() {
  ...
  try {
    ...
    urlAllowlistUpstreamHostsInput.value = (
      settings.url_allowlist_upstream_hosts || []
    ).join("\n");
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      url_allowlist_enabled: form.url_allowlist_enabled,
      url_allowlist_upstream_hosts: parseURLAllowlistHostsInput(
        urlAllowlistUpstreamHostsInput.value,
      ),
    ...
    urlAllowlistUpstreamHostsInput.value = (
      updated.url_allowlist_upstream_hosts || []
    ).join("\n");
    tablePageSizeOptionsInput.value = formatTablePageSizeOptions(
