...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, settingService)
	...
	codexRegistrationCandidateRepository := repository.NewCodexRegistrationCandidateRepository(client)
	codexRegistrationAccountReader := service.ProvideCodexRegistrationAccountReader(accountRepository)
	codexRegistrationOAuthRefresher := service.ProvideCodexRegistrationOAuthRefresher(openAIOAuthService)
	codexRegistrationAccountCreator := service.ProvideCodexRegistrationAccountCreator(adminService)
	codexRegistrationService := service.NewCodexRegistrationService(configConfig, codexRegistrationCandidateRepository, codexRegistrationAccountReader, codexRegistrationOAuthRefresher, codexRegistrationAccountCreator)
	openAIOAuthHandler := admin.NewOpenAIOAuthHandler(openAIOAuthService, adminService, codexRegistrationService)
	codexRegistrationHandler := handler.ProvideCodexRegistrationHandler(codexRegistrationService, codexRegistrationCandidateRepository)
	openAIOAuthDetectionService := service.NewOpenAIOAuthDetectionService(accountRepository, httpUpstream, tlsFingerprintProfileService)
	openAIOAuthDetectionHandler := handler.ProvideOpenAIOAuthDetectionHandler(openAIOAuthDetectionService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, codexRegistrationHandler, openAIOAuthDetectionHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, paymentHandler, affiliateHandler)
	accountRegistrationWorkerOAuthRefresher := service.ProvideAccountRegistrationWorkerOAuthRefresher(openAIOAuthService)
	accountRegistrationWorkerAccountReader := service.ProvideAccountRegistrationWorkerAccountReader(accountRepository)
	accountRegistrationWorkerAccountCreator := service.ProvideAccountRegistrationWorkerAccountCreator(adminService)
	accountRegistrationWorkerProxyReader := service.ProvideAccountRegistrationWorkerProxyReader(adminService)
	accountRegistrationWorkerModelFetcher := service.ProvideAccountRegistrationWorkerModelFetcher()
	accountRegistrationWorkerService := service.ProvideAccountRegistrationWorkerService(configConfig, accountRegistrationWorkerOAuthRefresher, accountRegistrationWorkerAccountReader, accountRegistrationWorkerAccountCreator, accountRegistrationWorkerProxyReader, accountRegistrationWorkerModelFetcher)
	accountRegistrationWorkerHandler := handler.ProvideAccountRegistrationWorkerHandler(accountRegistrationWorkerService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, accountRegistrationWorkerHandler, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, idempotencyCoordinator, idempotencyCleanupService)
