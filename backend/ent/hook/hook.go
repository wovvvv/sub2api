...
// The CodexRegistrationCandidateFunc type is an adapter to allow the use of ordinary
// function as CodexRegistrationCandidate mutator.
type CodexRegistrationCandidateFunc func(context.Context, *ent.CodexRegistrationCandidateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CodexRegistrationCandidateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CodexRegistrationCandidateMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CodexRegistrationCandidateMutation", m)
}
