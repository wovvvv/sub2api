...
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact bool) ([]byte, bool, error) {
	...
	if serviceTier := gjson.GetBytes(normalized, "service_tier"); serviceTier.Exists() {
		normalizedServiceTier := normalizeOpenAIServiceTier(serviceTier.String())
		if normalizedServiceTier == nil {
			...
			normalized = next
			changed = true
		} else if serviceTier.String() != *normalizedServiceTier {
			next, err := sjson.SetBytes(normalized, "service_tier", *normalizedServiceTier)
			if err != nil {
				return body, false, fmt.Errorf("normalize passthrough body service_tier=%s: %w", *normalizedServiceTier, err)
			}
			normalized = next
			changed = true
		}
