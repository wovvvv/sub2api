...
type cachedUpstreamURLAllowlistSettings struct {
	value     config.URLAllowlistConfig
	expiresAt int64
}

var upstreamURLAllowlistCache atomic.Value // *cachedUpstreamURLAllowlistSettings
var upstreamURLAllowlistSF singleflight.Group

const upstreamURLAllowlistCacheTTL = 60 * time.Second
const upstreamURLAllowlistErrorTTL = 5 * time.Second
...
func (s *SettingService) GetEffectiveUpstreamURLAllowlist(ctx context.Context) config.URLAllowlistConfig {
	base := config.URLAllowlistConfig{
		Enabled:           false,
		AllowPrivateHosts: true,
		AllowInsecureHTTP: true,
	}
	if s != nil && s.cfg != nil {
		base = s.cfg.Security.URLAllowlist
		base.UpstreamHosts = normalizeURLAllowlistHosts(base.UpstreamHosts)
	}
	if s == nil || s.settingRepo == nil {
		return base
	}
	if cached, ok := upstreamURLAllowlistCache.Load().(*cachedUpstreamURLAllowlistSettings); ok {
		if time.Now().UnixNano() < cached.expiresAt {
			return cached.value
		}
	}

	resolved, _, _ := upstreamURLAllowlistSF.Do("upstream_url_allowlist", func() (any, error) {
		values, err := s.settingRepo.GetMultiple(ctx, []string{
			SettingKeyURLAllowlistEnabled,
			SettingKeyURLAllowlistUpstreamHosts,
		})
		if err != nil {
			upstreamURLAllowlistCache.Store(&cachedUpstreamURLAllowlistSettings{
				value:     base,
				expiresAt: time.Now().Add(upstreamURLAllowlistErrorTTL).UnixNano(),
			})
			return base, err
		}

		effective := base
		if raw, ok := values[SettingKeyURLAllowlistEnabled]; ok {
			effective.Enabled = raw == "true"
		}
		if raw, ok := values[SettingKeyURLAllowlistUpstreamHosts]; ok && strings.TrimSpace(raw) != "" {
			effective.UpstreamHosts = parseStoredURLAllowlistHosts(raw)
		}
		effective.AllowPrivateHosts = s.effectiveAllowPrivateHosts()
		effective.AllowInsecureHTTP = s.effectiveAllowInsecureHTTP()
		upstreamURLAllowlistCache.Store(&cachedUpstreamURLAllowlistSettings{
			value:     effective,
			expiresAt: time.Now().Add(upstreamURLAllowlistCacheTTL).UnixNano(),
		})
		return effective, nil
	})
	if cfgValue, ok := resolved.(config.URLAllowlistConfig); ok {
		return cfgValue
	}
	return base
}

func (s *SettingService) effectiveAllowPrivateHosts() bool {
	if s != nil && s.cfg != nil {
...
func (s *SettingService) effectiveAllowInsecureHTTP() bool {
	if s != nil && s.cfg != nil {
		return s.cfg.Security.URLAllowlist.AllowInsecureHTTP
	}
	return true
}
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeyURLAllowlistEnabled] = strconv.FormatBool(settings.URLAllowlistEnabled)
	urlAllowlistHostsJSON, err := json.Marshal(normalizeURLAllowlistHosts(settings.URLAllowlistUpstreamHosts))
	if err != nil {
		return nil, fmt.Errorf("marshal url allowlist upstream hosts: %w", err)
	}
	updates[SettingKeyURLAllowlistUpstreamHosts] = string(urlAllowlistHostsJSON)
...
func (s *SettingService) refreshCachedSettings(settings *SystemSettings) {
	...
	upstreamURLAllowlistCache.Store(&cachedUpstreamURLAllowlistSettings{
		value: config.URLAllowlistConfig{
			Enabled:           settings.URLAllowlistEnabled,
			UpstreamHosts:     normalizeURLAllowlistHosts(settings.URLAllowlistUpstreamHosts),
			AllowPrivateHosts: s.effectiveAllowPrivateHosts(),
			AllowInsecureHTTP: s.effectiveAllowInsecureHTTP(),
		},
		expiresAt: time.Now().Add(upstreamURLAllowlistCacheTTL).UnixNano(),
	})
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	emailVerifyEnabled := settings[SettingKeyEmailVerifyEnabled] == "true"
	baseURLAllowlist := config.URLAllowlistConfig{}
	if s.cfg != nil {
		baseURLAllowlist = s.cfg.Security.URLAllowlist
		baseURLAllowlist.UpstreamHosts = normalizeURLAllowlistHosts(baseURLAllowlist.UpstreamHosts)
	}
	result := &SystemSettings{
		...
		InvitationCodeEnabled:            settings[SettingKeyInvitationCodeEnabled] == "true",
		TotpEnabled:                      settings[SettingKeyTotpEnabled] == "true",
		URLAllowlistEnabled:              baseURLAllowlist.Enabled,
		URLAllowlistUpstreamHosts:        baseURLAllowlist.UpstreamHosts,
	...
	if raw, ok := settings[SettingKeyURLAllowlistEnabled]; ok {
		result.URLAllowlistEnabled = raw == "true"
	}
	if raw, ok := settings[SettingKeyURLAllowlistUpstreamHosts]; ok && strings.TrimSpace(raw) != "" {
		result.URLAllowlistUpstreamHosts = parseStoredURLAllowlistHosts(raw)
	}
...
func parseStoredURLAllowlistHosts(raw string) []string {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return nil
	}
	...
	if err := json.Unmarshal([]byte(raw), &values); err == nil {
		return normalizeURLAllowlistHosts(values)
	}
	return normalizeURLAllowlistHosts([]string{raw})
}

func normalizeURLAllowlistHosts(values []string) []string {
	if len(values) == 0 {
		return nil
	}
	...
	for _, value := range values {
		for _, token := range splitURLAllowlistTokens(value) {
			host := normalizeURLAllowlistHost(token)
			if host == "" {
				continue
			}
...
func splitURLAllowlistTokens(value string) []string {
	return strings.FieldsFunc(value, func(r rune) bool {
		return r == ',' || r == '，' || r == '\n' || r == '\r'
	})
}

func normalizeURLAllowlistHost(raw string) string {
	entry := strings.ToLower(strings.TrimSpace(raw))
	if entry == "" {
		return ""
	}
	if strings.HasPrefix(entry, "*.") {
		suffix := strings.TrimSuffix(strings.Trim(strings.TrimPrefix(entry, "*."), "/"), ".")
		if suffix == "" {
			return ""
		}
		return "*." + suffix
	...
	if strings.Contains(entry, "://") {
		if parsed, err := url.Parse(entry); err == nil {
			entry = strings.ToLower(strings.TrimSpace(parsed.Hostname()))
		}
	}
	entry = strings.TrimSuffix(strings.Trim(entry, "/"), ".")
