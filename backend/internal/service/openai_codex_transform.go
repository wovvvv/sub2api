...
func applyCodexOAuthTransform(reqBody map[string]any, isCodexCLI bool, isCompact bool) codexTransformResult {
	...
	if rawServiceTier, ok := reqBody["service_tier"].(string); ok {
		normalizedServiceTier := normalizeOpenAIServiceTier(rawServiceTier)
		switch {
		case normalizedServiceTier == nil:
			delete(reqBody, "service_tier")
			result.Modified = true
		case *normalizedServiceTier != strings.TrimSpace(rawServiceTier):
			reqBody["service_tier"] = *normalizedServiceTier
			result.Modified = true
		}
	} else if _, ok := reqBody["service_tier"]; ok {
		delete(reqBody, "service_tier")
		result.Modified = true
	}
