...
var officialOpenAIModelsURL = "https://api.openai.com/v1/models"
...
type ImportOpenAIModelsRequest struct {
...
type ImportOpenAIModelsResponse struct {
...
// ImportOpenAIModels imports model ids from an OpenAI-compatible /v1/models endpoint.
// POST /api/v1/admin/accounts/import-openai-models
func (h *AccountHandler) ImportOpenAIModels(c *gin.Context) {
	var req ImportOpenAIModelsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	account, err := h.resolveOpenAIModelsImportAccount(c.Request.Context(), req)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	models, fallback, err := h.importOpenAIModels(c.Request.Context(), account, req)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	response.Success(c, ImportOpenAIModelsResponse{
		Models:   models,
		Fallback: fallback,
	})
}

func (h *AccountHandler) resolveOpenAIModelsImportAccount(ctx context.Context, req ImportOpenAIModelsRequest) (*service.Account, error) {
	if req.AccountID != nil {
		...
		if account == nil || !account.IsOpenAI() {
			return nil, infraerrors.BadRequest("INVALID_ACCOUNT", "account must be an OpenAI account")
		}
		return account, nil
	}

	if req.Platform != service.PlatformOpenAI {
		return nil, infraerrors.BadRequest("INVALID_IMPORT_TARGET", "only OpenAI accounts are supported")
	}
	...
	if req.Type != service.AccountTypeAPIKey {
		return nil, infraerrors.BadRequest("INVALID_IMPORT_TARGET", "unsupported OpenAI account type")
	}
	if strings.TrimSpace(req.APIKey) == "" {
		return nil, infraerrors.BadRequest("API_KEY_REQUIRED", "api_key is required")
	}
...
func (h *AccountHandler) importOpenAIModels(ctx context.Context, account *service.Account, req ImportOpenAIModelsRequest) ([]string, bool, error) {
	if account == nil || !account.IsOpenAI() {
		return nil, false, infraerrors.BadRequest("INVALID_ACCOUNT", "account must be an OpenAI account")
	}

	if account.IsOpenAIOAuth() {
		// OpenAI OAuth accounts in sub2api use the ChatGPT/Codex flow.
		// Those access tokens commonly lack the api.model.read scope required by
		// api.openai.com/v1/models, so treat the built-in OpenAI model set as the
		// canonical import source instead of reporting a misleading fallback.
		return openai.DefaultModelIDs(), false, nil
	}
	if !account.IsOpenAIApiKey() {
		return nil, false, infraerrors.BadRequest("UNSUPPORTED_ACCOUNT_TYPE", "only OpenAI API key and OAuth accounts are supported")
	}
	...
	if apiKey == "" {
		return nil, false, infraerrors.BadRequest("API_KEY_REQUIRED", "api_key is required")
	}

	modelsURL, err := h.resolveOpenAIModelsURL(baseURL)
	if err != nil {
		return nil, false, err
	}
	models, err := h.fetchOpenAIModels(ctx, modelsURL, apiKey)
	if err != nil {
		return nil, false, err
	}
...
func (h *AccountHandler) resolveOpenAIModelsURL(baseURL string) (string, error) {
	...
	if h.accountTestService != nil {
		normalized, err := h.accountTestService.ValidateUpstreamBaseURL(baseURL)
		if err != nil {
			return "", infraerrors.BadRequest("INVALID_BASE_URL", err.Error())
		}
		baseURL = normalized
	}
...
func (h *AccountHandler) fetchOpenAIModels(ctx context.Context, modelsURL, bearerToken string) ([]string, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, modelsURL, nil)
	if err != nil {
		return nil, infraerrors.BadRequest("MODEL_IMPORT_FAILED", err.Error())
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, infraerrors.BadRequest("MODEL_IMPORT_FAILED", fmt.Sprintf("upstream returned %d: %s", resp.StatusCode, strings.TrimSpace(string(body))))
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, infraerrors.BadRequest("MODEL_IMPORT_FAILED", "invalid models response")
	}
	...
	if len(models) == 0 {
		return nil, infraerrors.BadRequest("MODEL_IMPORT_FAILED", "no models returned by upstream")
	}
	return models, nil
}
